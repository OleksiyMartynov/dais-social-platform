...
contract("VoteStation", accounts => {
    ...
    it("should not allow votes if unauthorized address", async () => {
        ...
        let failed = false;
        try {
            let tx1 = await meta.vote(1, MOCK_VOTER_1_FOR, MOCK_VOTER_1, { from: MOCK_UNAUTHARIZED_CONTRACT_ADDRESS, value: MOCK_VOTER_1_VOTES });
        } catch (ex) {
            failed = true;
        }
        assert(failed, "should have thrown an exception");
    ...
    it("should allow votes before vote end time", async () => {
        ...
        for (let i = 0; i < MOCK_VOTERS.length; i++) {
            ...
            let tx = await meta.vote(1, MOCK_VOTERS_FOR[i], MOCK_VOTERS[i], { value: MOCK_VOTER_VOTES[i] });
            assert(tx.receipt.status, "transaction failed");
    ...
    it("should not allow votes if already voted", async () => {
        ...
        for (let i = 0; i > MOCK_VOTERS.length; i++) {
            let failed = false;
            try {
                let tx1 = await meta.vote(1, MOCK_VOTERS_FOR[i], MOCK_VOTERS[i], { value: MOCK_VOTER_VOTES[i] });
            } catch (ex) {
                failed = true;
            }
            assert(failed, "should have thrown an exception");
